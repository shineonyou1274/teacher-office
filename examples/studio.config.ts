...
//  실제 하루를 그대로 옮긴 것입니다.
//
//  쓰려면 npm run use -- studio 를 치세요.
//    setup 에서 적으신 이름표(학교·이름·호칭·과목)는 그대로 따라옵니다.
//    되돌릴 때는 npm run use -- 되돌리기.
//
//  이 예시의 특징
...
//   - 주로 쓰는 검수 기준은 **C (강의·연수 자료)** 입니다.
//       행정 문서(생기부·공문)를 안 만드는 하루라 A는 거의 안 씁니다
//
//  하루 순서를 이렇게 정한 이유 (나중에 직접 고치실 때 근거가 됩니다)
//   1. 자료는 웹보다 내 드라이브가 먼저입니다. 이미 만든 걸 또 만들지 않으려고요.
//   2. 1차 추리기는 사람 자리인데, 제일 오래 걸리고 제일 미루게 되는 자리이기도
//      합니다. 그래서 앞의 웹 자료팀에 "올릴 것만 추리는" 사람을 한 명 붙여
//      넘어오는 양을 줄였고, 화면에서도 그 줄이 제일 길게 머물게 했습니다.
//   3. 결재는 하루 한 번, 흐름안 고르기 자리 하나뿐입니다. 되돌리기가 제일
//      비싼 지점이 흐름이라서, 만들기 전에 고르고 들어갑니다.
// ══════════════════════════════════════════════════════════
